...
hyperparameters = {
    'ant-expert-v2':       {'lr': 3e-4, 'eta': 1.0,   'max_q_backup': False,  'reward_tune': 'no',          'eval_freq': 5, 'num_epochs': 75, 'gn': 5.0,  'top_k': 2, 'multi_task': True, 'trainset_folder': "ant_dir/", 'trainset_name': "ant_dir-*-expert", 'unlearn_task': 0},
}
...
def train_agent(env, env_id, state_dim, action_dim, max_action, device, output_dir, args, skip_eval=False, retrain=False, finetune=False, rand_reward=False, traj_deleter=False, erase_diff=False, per_data=1):
    ...
    #if (args.unlearn):
    #    agent.load_model(dir='./agents_copy/ant-dir-pretrain/', id=env_id)
    #    env_id = env_id + '_unlearn_task' + str(args.unlearn_task)
    if ul_bool:
        print("Loading a pre-trained model!")
        agent.load_model(dir='./models/multi/pretrain/', id=env_id+'_pretrain')
...
def eval_policy_dir(policy, env_name, seed, eval_episodes=1, max_ep_len=200):
    scores = []
    for i in range(50):
        if i%2==0:
            task_paths = f"../prompt-dt/config/ant_dir/config_ant_dir_task{i}.pkl"
            tasks = []
            with open(task_paths.format(i), 'rb') as f:
                task_info = pickle.load(f)
                assert len(task_info) == 1, f'Unexpected task info: {task_info}'
                tasks.append(task_info[0])
            eval_env = AntDirEnv(tasks, len(tasks), include_goal = False)
...
if __name__ == "__main__":
    ...
    task_paths = f"../prompt-dt/config/ant_dir/config_ant_dir_task0.pkl"
    tasks = []
    with open(task_paths.format(0), 'rb') as f:
        task_info = pickle.load(f)
        assert len(task_info) == 1, f'Unexpected task info: {task_info}'
        tasks.append(task_info[0])
    env = AntDirEnv(tasks, len(tasks))
    ...
    state_dim = env.observation_space.shape[0] + 25 # +25 for the dir dim
